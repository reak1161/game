...
def tail_rush(enemy):
    ...
    # 攻撃
    point = [['attack', 150, 'Normal', scope, 0, 5, 0],
             ['attack', 150, 'Normal', scope, 1, 5, 0],
             ['attack', 150, 'Normal', scope, 2, 5, 0],
             ['attack', 150, 'Normal', scope, 3, 5, 0]]
...
def snake_dash(enemy):
    ...
    for i in range(4):
        ...
        if i % 2 == 0:
            for j in range(4):
                point.append(['attack', 75, 'Normal', j, i, 5+j*0.05+i*0.20, j*0.05+i*0.20])
        # 奇数行は逆方向
        else:
            for j in range(4-1, -1, -1):
                point.append(['attack', 75, 'Normal', j, i, 5+(3-j)*0.05+i*0.20, (3-j)*0.05+i*0.20])
...
def tightening(enemy):
    ...
    # バフデバフはリストで末尾につける？
    # 攻撃は２次元配列で与える
    point = [['attack', 50, 'Normal', scope_x, scope_y, 5, 0, ['debuff', 'cant_move', 10], ['debuff', 'cant_action', 10], ['debuff', 'slip', 10, 1, 1, 10]]]

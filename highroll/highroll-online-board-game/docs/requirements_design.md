...
- **デッキ**: `data/decklist.*.json` で定義。既定は `decklist.default_60.json`、拡張例として 90 枚の `decklist.expanded_mixed_90.json` を用意する。
---
...
- 丸め規則: 割合/倍率などで小数が出た場合、明記がない限り **切り捨て（floor）**。
...
- **効率**: すべてのアイテム (カード) 効果が 2 倍になる。
- **貪欲**: 追加トークン獲得時、獲得予定のトークン数だけ HP を回復する。
- **延期**: 最初の行動後、残りの Bra を全員の行動後に使用できる。
...
- カード効果が補正される場合は、元の数値に取り消し線を付け、補正後の数値を色分け表示する（増加は赤、減少は青）。

---

## 14. 将来：Workers + Durable Objects でのオンライン化（方針）
- 現状はローカル（WSL）で Node/Vite を起動し、Cloudflare Tunnel で外部に見せる方式。
- 将来的には **Cloudflare Pages + Workers + Durable Objects + WebSocket** で「クラウド上で完結」させる。
  - ポーリングはリクエスト数を消費しやすいので避け、**WS常時接続 + イベント駆動（差分配信）**を基本にする。
  - `socket.io` は Workers へそのまま移植しづらいので、**標準 WebSocket + JSON メッセージ**へ置き換える方針。
  - 詳細は `docs/online_workers_do.md` を参照。

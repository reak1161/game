...
class GameEngine {
    ...
    previewDamageOutcome(targetId, attackerId, incoming, source) {
    ...
    constructor(matchId, initialPlayers = []) {
        this.lastDefeatContext = null;
        this.lastNonSelfDefeatTargetId = null;
        this.cpuPlayers = new Map();
        this.cpuLoopActive = false;
        this.cpuTimer = null;
        this.cpuNextAt = 0;
        // CPUの行動が速すぎると何が起きたか分からないため、最低間隔を設ける（ms）
        this.cpuActionDelayMs = 1000;
        this.cursePool = [
            'weakness',
            'force',
            'decay',
        ...
        this.transientCardEffectBonus = new Map();
    ...
    registerCpuPlayer(playerId, level = 'normal') {
        this.cpuPlayers.set(playerId, level);
    }
    isCpuPlayer(playerId) {
        if (!playerId)
            return false;
        return this.cpuPlayers.has(playerId);
    }
    decideCpuPromptAcceptance(playerId, prompt) {
    ...
    playCard(playerId, cardId, options) {
        ...
        const originalDiscardIndex = needsRebuttalDiscard && typeof curseDiscardIndexRaw === 'number' && Number.isFinite(curseDiscardIndexRaw)
        ...
        if (misfire) {
            ...
            this.state = (0, gameState_1.consumeBra)(this.state, playerId, totalBraCost);
    ...
    discardHandCardToSharedDiscard(playerId, index) {
        const hand = [...(this.state.hands[playerId] ?? [])];
    ...
    applyWitchSpellPassive(playerId, card) {
    ...
    applyCursesOnCardUse(playerId, _card, curseId) {
        if (!curseId) {
            return;
        }
        if (curseId === 'cost') {
    ...
    applyResonanceMirrorEffects(playerId, card, trigger, options) {
        card.effects?.forEach((effect, index) => {
            ...
            const cloned = { ...effect, target: 'self' };
    ...
    rescueBra(playerId) {
        ...
        const maxBra = (0, effectUtils_1.getEffectiveStatValue)(runtime, 'bra');
        if (maxBra > 0) {
        ...
        this.state = (0, gameState_1.setBraTokens)(this.state, playerId, currentBraTokens + 1);
        this.logEvent({
            type: 'roleAction',
            playerId,
            actionId: 'rescue_bra',
    ...
    roleAttack(playerId, targetId, options) {
        ...
        const atk = (0, effectUtils_1.getEffectiveStatValue)(attackerRuntime, 'atk') + Math.max(0, Math.floor(nextRoleAttackAtkBonus));
    ...
    finishRoleAttack(attackerId, targetId, inflicted, isStruggle) {
        ...
        if (!isStruggle) {
            this.state = (0, gameState_1.consumeBra)(this.state, attackerId, 1);
        }
        ...
        if (attacker?.roleId === 'barrage' && !this.isRoleSuppressed(attackerId)) {
            ...
            this.state = (0, gameState_1.updatePlayerRuntimeState)(this.state, attackerId, (runtime) => {
            ...
            if (!isStruggle && firstOnTarget) {
                ...
                this.state = (0, gameState_1.setBraTokens)(this.state, attackerId, currentBra + 1);
            }
        }
    ...
    applyDefenderAfterRoleAttackInstallEffects(attackerId, targetId) {
    ...
    applyAttackerAfterRoleAttackInstallEffects(attackerId, targetId, dealt) {
        if (attackerId === targetId) {
            return;
        }
        const attackerRuntime = this.getRuntime(attackerId);
        ...
        for (const install of attackerRuntime.installs) {
            ...
            for (const effect of card.effects) {
                ...
                const baseValue = typeof effect.valueMultiplierOfDealt === 'number'
    ...
    resolveResonateRoleAttack(playerId, targetId, isStruggle, resume) {
        const attackerRuntime = this.ensureRuntimeExists(playerId);
        const defenderRuntime = this.ensureRuntimeExists(targetId);
    ...
    finalizePendingAction(action, dealt) {
        if (!action) {
            return;
        }
    ...
    getResonateDecayRatio(attackerId) {
    ...
    resolvePendingPrompt(playerId, accepted) {
        ...
        const applied = this.applyDamageToPlayer(pending.attackerId ?? pending.targetId, pending.targetId, pending.amount, pending.source, {
    ...
    executeBombAction(playerId, actionId, targetId) {
    ...
    executeVampireAction(playerId, actionId, choices) {
        ...
        const rawIndex = choices?.handIndex;
    ...
    executeJesterAction(playerId, actionId) {
        ...
        let cursor = 0;
        const dealAbilityDamage = (targetId, amount) => {
        ...
        const applyEffect = (chance, handler) => {
        ...
        if (applyEffect(3, () => {
            const currentBra = this.state.braTokens[playerId] ?? 0;
            if (currentBra > 0) {
                this.state = (0, gameState_1.setBraTokens)(this.state, playerId, Math.max(0, currentBra - 1));
            }
        })) {
            return '道化: Bra-1';
        }
        ...
        if (applyEffect(3, () => {
            ...
            this.state = (0, gameState_1.drawFromSharedDeck)(this.state, playerId, hand.length);
            this.syncHandStatTokens(playerId);
        })) {
    ...
    executeSuppressAction(playerId, actionId, targetId) {
    ...
    executeShedAction(playerId, actionId) {
        ...
        const currentDef = Math.max(0, (0, effectUtils_1.getEffectiveStatValue)(runtime, 'def'));
        ...
        if (runtime.turnBoosts.def !== 0) {
            this.state = (0, gameState_1.updatePlayerRuntimeState)(this.state, playerId, (next) => {
                if (!next) {
    ...
    executeSealAction(playerId, actionId, targetId) {
        switch (actionId) {
            case 'seal_chain_atk':
    ...
    executeWitchAction(playerId, actionId, targetId) {
    ...
    start() {
        ...
        order.forEach((playerId) => {
            const runtime = this.getRuntime(playerId);
            const baseBra = runtime ? (0, effectUtils_1.getEffectiveStatValue)(runtime, 'bra') : this.getBra(this.getPlayer(playerId)?.roleId);
            this.state = (0, gameState_1.setBraTokens)(this.state, playerId, Math.max(0, baseBra));
    ...
    endTurn(playerId) {
        ...
        const roleId = this.getPlayer(playerId)?.roleId;
        const shouldSplitTurn = (postponePhase === 'acted' || postponePhase === 'idle') &&
            !isDeferredTurn &&
    ...
    applyCollapseCurseAtTurnEnd(playerId) {
        const runtime = this.getRuntime(playerId);
        if (!runtime || runtime.isDefeated) {
            return;
        }
    ...
    performCpuTurn(playerId) {
        ...
        const candidates = hand
            ...
            .filter((entry) => Boolean(entry.card && entry.card.playable !== false))
        ...
        const chooseTargetsAndChoices = (card) => {
            const effectTarget = (e) => {
                if (!('target' in e))
                    return undefined;
                return e.target;
            };
            ...
            if (needsStat) {
                const options = ['atk', 'def', 'spe'];
                const stat = level === 'easy'
        ...
        const scoreCard = (card) => {
            ...
            for (const effect of card.effects) {
                if (effect.type === 'dealDamage') {
                    ...
                    const v = typeof effect.value === 'number'
                        ? effect.value
                        : effect.valueFormula
                            ? 2
                            : 1;
    ...
    applySetNextRoleAttackAtkBonusEffect(playerId, effect) {
    ...
    applyDealDamageEffect(playerId, card, effect, options) {
        ...
        targets.forEach((targetId) => {
            ...
            const kindLabel = effect.defApplied && !effect.ignoreDef ? '通常ダメージ' : effect.fixed || effect.ignoreDef ? '固定ダメージ' : 'ダメージ';
    ...
    applyBrokenWindowTheoryEffect(playerId, card, _effect) {
        const alive = this.state.players
            .map((player) => ({ player, runtime: this.getRuntime(player.id) }))
            .filter((entry) => entry.runtime && !entry.runtime.isDefeated);
        if (alive.length === 0) {
            return;
        }
    ...
    applyPoltergeistEffect(playerId, card, effect, options) {
        ...
        const removed = Math.max(0, runtime.statTokens.atk) +
        ...
        this.state = (0, gameState_1.updatePlayerRuntimeState)(this.state, playerId, (existing) => {
    ...
    applySelfInstallEffect(playerId, card) {
        this.reclaimCardFromDiscard(card.id);
        this.installCard(playerId, card);
    }
    applyDrawCardsEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        ...
        targets.forEach((targetId) => {
            ...
            this.state = (0, gameState_1.drawFromSharedDeck)(this.state, targetId, count);
    ...
    applyAdjustBraEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        ...
        targets.forEach((targetId) => {
            ...
            this.state = (0, gameState_1.setBraTokens)(this.state, targetId, next);
            if (deltaApplied !== 0) {
                this.logEvent({
                    type: 'cardEffect',
                    playerId,
    ...
    applyStatDebuffUntilDamageEffect(playerId, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = effect.value * multiplier + bonus;
        const targets = this.resolveTargets(effect.target, playerId, options?.targets);
    ...
    applyBurnEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = effect.value * multiplier + bonus;
        if (adjusted === 0) {
    ...
    applyBleedEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = effect.value * multiplier + bonus;
        if (adjusted === 0) {
    ...
    applyShockEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
    ...
    applyDizzyEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = Math.floor(effect.value * multiplier + bonus);
        if (adjusted <= 0) {
    ...
    applyTauntUntilNextTurnStartEffect(playerId, effect) {
    ...
    applySealHandEffect(playerId, card, effect, options) {
        if (effect.mode !== 'all') {
            return;
        }
        const targets = this.resolveTargets(effect.target ?? 'self', playerId, options?.targets);
    ...
    applyDealDamagePerSealedHandEffect(playerId, card, effect) {
        ...
        const rounded = roundMode === 'ceil' ? Math.ceil(raw) : roundMode === 'round' ? Math.round(raw) : Math.floor(raw);
    ...
    applyChooseOneEffect(playerId, card, effect, options) {
        const player = this.getPlayer(playerId);
        if (player?.roleId === 'strong_greed') {
            const raw = options?.choices?.[effect.key];
            const rawSelections = raw && typeof raw === 'object'
                ? (raw.selections ?? raw)
                : null;
            const baseChoices = { ...(options?.choices ?? {}) };
            delete baseChoices[effect.key];
            effect.options.forEach((option) => {
                const selection = rawSelections && typeof rawSelections === 'object' ? rawSelections[option.value] : null;
                const selectionTargets = Array.isArray(selection?.targets)
                    ? selection.targets.filter((t) => typeof t === 'string')
                    : undefined;
                ...
                const childOptions = {
                    ...
                    choices: selectionChoices ? { ...baseChoices, ...selectionChoices } : baseChoices,
                };
        ...
        const selected = effect.options.find((opt) => opt.value === chosenValue) ??
    ...
    applyStunEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        ...
        targets.forEach((targetId) => {
            ...
            const currentSpe = (0, effectUtils_1.getEffectiveStatValue)(runtime, 'spe');
    ...
    applyHealEffect(playerId, card, effect, options) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = effect.value * multiplier + bonus;
        if (adjusted <= 0) {
    ...
    applyModifyMaxHpInstallEffect(playerId, card, effect) {
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
        const adjusted = effect.value * multiplier + bonus;
    ...
    applyModifyTurnOrderEffect(effect) {
    ...
    getSortedAliveTurnOrder(mode) {
        return this.state.players
            ...
            .slice()
            .sort((a, b) => {
            const speA = (0, effectUtils_1.getEffectiveStatValue)(this.getRuntime(a.id), 'spe');
            const speB = (0, effectUtils_1.getEffectiveStatValue)(this.getRuntime(b.id), 'spe');
            return mode === 'ascendingSpe' ? speA - speB : speB - speA;
        })
            .map((player) => player.id);
    }
    applyNextRoundPriority(order, mode, nextRound) {
    ...
    resolveNextRoundMode(nextRound) {
        const currentRound = Number.isFinite(this.state.round) ? this.state.round : 1;
    ...
    reorderTurnOrder(mode, preferredCurrentId) {
    ...
    applyAddStatTokenEffect(playerId, card, effect, options) {
        const actorRuntime = this.getRuntime(playerId);
        const multiplier = this.getCardEffectMultiplier(playerId);
        const bonus = this.getCardEffectBonus(playerId);
    ...
    applyDiscardAllHandEffect(playerId, card, effect, options) {
        const targets = this.resolveTargets(effect.target, playerId, options?.targets);
        targets.forEach((targetId) => {
            if (!this.isEffectConditionSatisfied(effect.condition, playerId, targetId)) {
                return;
    ...
    applyDiscardThenDrawEffect(playerId, card, effect, options) {
        ...
        targets.forEach((targetId) => {
            ...
            if (discarded.length > 0) {
                ...
                this.state = (0, gameState_1.drawFromSharedDeck)(this.state, targetId, discarded.length);
    ...
    clearBloodPatternHand(playerId) {
        this.updateRoleState(playerId, (prev) => {
    ...
    shiftSealedHandAfterRemoval(playerId, removedIndex) {
        if (removedIndex < 0) {
            return;
        }
        this.updateRoleState(playerId, (prev) => {
    ...
    shiftCursedHandAfterRemoval(playerId, removedIndex) {
        if (removedIndex < 0) {
            return;
        }
        this.updateRoleState(playerId, (prev) => {
    ...
    shiftBloodPatternHandAfterRemoval(playerId, removedIndex) {
        if (removedIndex < 0) {
            return;
        }
        this.updateRoleState(playerId, (prev) => {
    ...
    applyCoinFlipDealDamageEffect(playerId, card, effect, options) {
    ...
    applyCoinFlipDealDamageEitherEffect(playerId, card, effect, options) {
        ...
        const chosenTarget = effect.target;
    ...
    applySetNextRoundPriorityEffect(playerId, _effect) {
        const currentRound = Number.isFinite(this.state.round) ? this.state.round : 1;
        this.state = {
            ...this.state,
    ...
    applyAdrenalineEffect(playerId, effect) {
    ...
    syncHandStatTokens(playerId) {
        const runtime = this.ensureRuntimeExists(playerId);
        const prevTokens = runtime.roleState?.handStatTokens ?? {
            atk: 0,
            def: 0,
            spe: 0,
            bra: 0,
        };
        const nextTokens = { atk: 0, def: 0, spe: 0, bra: 0 };
        const hand = this.state.hands[playerId] ?? [];
        hand.forEach((cardId) => {
            const card = this.cardMap.get(cardId);
        ...
        Object.keys(nextTokens).forEach((stat) => {
    ...
    applyDoubleBaseStatEffect(playerId, card, effect, options) {
    ...
    reduceHpDirectly(playerId, amount) {
        if (amount <= 0) {
            return;
        }
        const runtime = this.getRuntime(playerId);
    ...
    applyDamageToPlayer(attackerId, targetId, amount, source = 'other', options) {
        if (amount <= 0) {
            return 0;
        }
        if (this.state.pendingPrompt) {
        ...
        if (isPostpone && !isDeferredPostponeTurn) {
            ...
            if (deferredAmount > 0) {
                ...
                this.updateRoleState(targetId, (prev) => ({
                    ...
                    postponeDeferredAttackerId: attackerId && attackerId !== targetId ? attackerId : prev.postponeDeferredAttackerId,
                }));
                this.logDamageResolved({
                    attackerId,
                    targetId,
    ...
    handleBeforeDamageEffects(targetId, amount, attackerId, source, options) {
        const runtime = this.getRuntime(targetId);
        if (!runtime) {
        ...
        if (runtime.installs.length > 0) {
            for (const install of runtime.installs) {
                ...
                for (const [effectIndex, effect] of card.effects.entries()) {
                    ...
                    const allowedSourcesForInstall = effect.sources ?? ['role', 'card'];
                    if (!allowedSourcesForInstall.includes(source)) {
                        continue;
                    }
                    const forcedDecision = options?.forcedPromptDecision &&
                    ...
                    if (effect.type === 'thresholdPrevent') {
                        ...
                        const shouldSkipOptional = options?.skipOptional &&
                    ...
                    if (effect.type === 'damageIntercept') {
                        ...
                        const max = typeof effect.max === 'number' ? Math.max(min, Math.floor(effect.max)) : Number.POSITIVE_INFINITY;
                        ...
                        const shouldKeep = typeof effect.freeIfDamageEquals === 'number' && incoming === effect.freeIfDamageEquals;
    ...
    handleInstallRemoved(playerId, cardId) {
    ...
    reconcileLightningRodAtkBonus(playerId) {
        const runtime = this.getRuntime(playerId);
        if (!runtime || runtime.isDefeated) {
            return;
        }
            ...
            return {
                ...
                installs: [
                    ...(card.subtype === 'equip'
                        ...
                        : next.installs),
                    { cardId: card.id, instanceId: (0, node_crypto_1.randomUUID)() },
                ],
            };
        });
    ...
    applyBeforeDamageAbilities(targetId, attackerId, amount, source) {
        ...
        abilities.forEach((ability) => {
            ...
            if (result.damageReduction) {
                ...
                const spentLabel = ability.id === 'swiftwind_spend_spe_reduce_damage' && abilityContext.spentStatTokens
    ...
    handleAfterDamageEvents(attackerId, targetId, amount, source) {
    ...
    getCardEffectMultiplier(playerId) {
        if (this.isRoleSuppressed(playerId)) {
            return 1;
        }
        const runtime = this.getRuntime(playerId);
    ...
    withTransientCardEffectBonus(playerId, bonus, fn) {
        if (bonus === 0) {
            return fn();
        }
    ...
    assertPostponeAllowsAction(playerId) {
        void playerId;
    }
    ...
    handlePostponeAfterAction(playerId) {
        const runtime = this.getRuntime(playerId);
        if (runtime && !runtime.isDefeated) {
    ...
    enqueueDeferredTurn(playerId, bra, allowZero = false) {
        if (bra <= 0 && !allowZero) {
            return;
        }
    ...
    popDeferredTurnForPlayer(playerId) {
        if (!this.state.deferredTurnActive) {
            return null;
        }
        const queue = this.state.deferredTurns ?? [];
    ...
    beginTurn(playerId) {
        ...
        this.state = (0, gameState_1.setBraTokens)(this.state, playerId, braToSet);
        const player = this.getPlayer(playerId);
        if (player?.roleId === 'postpone') {
            if (isDeferredTurn) {
                this.updateRoleState(playerId, (prev) => ({
    ...
    handleRoleEndTurnEffects(playerId) {
        ...
        if (timedBomb && typeof timedBomb.count === 'number' && Number.isFinite(timedBomb.count)) {
            ...
            if (next <= 0) {
                ...
                const applied = this.applyDamageToPlayer(timedBomb.sourcePlayerId, playerId, 10, 'ability', {
